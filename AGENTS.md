...
- Compact layout: OpenCode switches compact(mobile)/wide(desktop) on a `(min-width: 768px)` `matchMedia` query (unchanged in 1.18.x, incl. the redesigned desktop components). To force compact, patch `window.matchMedia` **before** the SPA bundle loads (`onLoadStart`) so `createMediaQuery` initializes with `matches: false` and never subscribes to real resize events. `forceCompactLayout` defaults **on**, so the panel normally renders the mobile layout — which keeps the **classic** review panel (`session-review-*`, `data-file`); the redesigned **v2** review panel (`session-review-v2-*`) only appears with `forceCompactLayout` off (desktop).
...
Since 1.18.0 (`desktop v2 migration finalising`) the redesigned layout is the **default** (`newLayoutDesigns` defaults true; the classic layout stays selectable until `oldInterfaceSunset`, 2026-09-14). The chat timeline that carries these diff targets is the shared `session-ui` `MessagePart`, rendered identically in compact and desktop, so Alt+Click diff nav is layout-independent (verified live vs 1.18.1 at both widths).

The Alt+Click "open diff in IDE" feature (`OpenCodeBrowserSnippets.buildDiffNavigationScript` → `features.OpenCodeDiffNavigation`) is the most DOM-coupled injection: it maps a click to a `(messageID, filePath)` pair from OpenCode SPA internals, none of which are a stable API and some of which have already shifted between releases. Re-verify against a real 1.18.1+ page whenever diffs stop opening:
...
The file-link handler (`buildFileLinkHandlerScript`) early-returns on `event.altKey`, reserving Alt for this gesture. Validated live vs 1.18.1 against the classic layout, the redesigned **compact** layout (the plugin's default; `forceCompactLayout` forces mobile, which keeps the classic `session-review-*`/`data-file` review panel), and — with `forceCompactLayout` **off** — the redesigned desktop **v2 review panel** (`session-review-v2-*`).

### Redesigned v2 review panel (file links with `forceCompactLayout` off)

The v2 review panel (desktop only) is `session-ui/src/v2/components/session-review-v2*`: a sidebar file tree + a preview pane, **replacing** the classic per-file "open" button. It has none of the classic `session-review-view-button`/`session-review-file-info`/`data-file` selectors. So `buildFileLinkHandlerScript` resolves the "open in IDE" path from the **preview header** — `[data-slot="session-review-v2-file-name"]` + optional `[data-slot="session-review-v2-file-path"]` (dir), both inside `[data-slot="session-review-v2-file-title"]`. It must **not** hijack the sidebar rows (`[data-slot="session-review-v2-sidebar-tree"] button[data-path]`): those are the SPA's own diff-preview navigation, and intercepting their click would break in-app review. (Diff Alt+Click from the review panel is inherently empty — the review is session-scoped, so no `[data-message-id]` ancestor → empty diff — in both v1 and v2; the meaningful diff nav is the chat timeline.)

### Open-project navigation contract (redesigned layout)

Verified live vs opencode 1.17.18 and 1.18.1 (where the redesigned layout is now the default). The redesigned layout (`settings.v3` → `general.newLayoutDesigns`, `body[data-new-layout]`) uses **directoryless routes**: `/server/<base64url(origin)>/session/<ses_...>` and `/new-session?draftId=...`. Directory routes (`/<base64url(dir)>/session[...]`) are accepted on a cold document load — and win over the SPA's own state — but are quickly rewritten to the directoryless form. Which project such a route shows is visible only in the SPA's `opencode.global.dat:server` localStorage (`lastProject`/`projects`), which the SPA does **not** update when it boots from a direct directory route. Consequences, each load-bearing in `buildOpenProjectScript` / `OpenCodeWebToolWindowContent`:

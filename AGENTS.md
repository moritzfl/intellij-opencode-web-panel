...
- Wire check after an OpenCode update: `OPENCODE_SERVER_PASSWORD=testpw123 scripts/check-wire-contract.sh http://127.0.0.1:<port> <project-directory>` verifies every consumed OpenAPI operation/operation ID is present and non-deprecated, required event names remain advertised, and live REST roots still have the expected object/array/envelope shapes. It is additive-field tolerant and deliberately does not pin the full OpenAPI document.

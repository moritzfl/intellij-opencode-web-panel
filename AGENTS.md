...
OpenCode's session-tab popover waits 2000ms (`titlebar-tab-popover` `OPEN_DELAY`). The panel clamps that Kobalte `window.setTimeout(..., 2000)` to 250ms while the pointer is on `[data-component="session-tab-popover-trigger"]`; the skip-window `0` delay is left alone. Home project rows (`[data-component="home-project-row"]`) have no worktree in the DOM — the overlay maps row order onto `opencode.global.dat:server` `projects` and skips when counts do not match (never guess by display name). It reuses `session-tab-popover` slots for page CSS. Toggle-off reloads.
...
- **Boot URL** — `buildServerSessionUrl(serverUrl, sessionId?)`, never `buildProjectUrl` (legacy directory route) and never the bare server root alone. Session choice is OpenCode's (filtered tabs / `lastProjectSession`). Do not REST-lookup `max(time.updated)` or JVM-navigate to "most recent". If the main document never reaches a successful `onLoadEnd` within 20s, retry the load up to twice (`OpenCodePageLoadWatchdog`). That watchdog must use its own Alarm — `onLoadStart` cancels `openProjectAlarm`. Do **not** `stopLoad` a navigation that is still inside the 20s budget.
...
Injection fixes and wire-contract checks must be validated against a live server — not `about:blank`, synthetic pages, or unit tests. DOM/wire contracts below last validated vs **opencode 1.18.25**; re-verify on OpenCode updates.
...
- **DOM gate** — `OPENCODE_SERVER_PASSWORD=testpw123 scripts/check-dom-contract.sh http://127.0.0.1:<port>` checks every DOM/JS marker used by injected scripts plus directly declared OpenCode Persist keys. The 1.18.25 baseline is **65 markers + 14 classified Persist keys**. Run it after changing selectors, SPA storage keys, media queries, icon names, command shortcuts, or injection targets. Add/change the marker or Persist classification in the same commit as the integration.
- **Wire gate** — `OPENCODE_SERVER_PASSWORD=testpw123 scripts/check-wire-contract.sh http://127.0.0.1:<port> <project-directory>` checks every consumed OpenAPI operation/operation ID for presence and deprecation, required event names, and live REST root/envelope shapes. The 1.18.25 baseline is **15 operations + 5 live roots + 9 event types**; the script self-tests every required operation with a negative mutation. Run it after changing REST/event consumers or protocol parsers, and update its operation/event/probe inventory in the same commit.

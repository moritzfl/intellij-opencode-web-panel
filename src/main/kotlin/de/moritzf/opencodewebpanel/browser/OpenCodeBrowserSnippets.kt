...
internal object OpenCodeBrowserSnippets {
    ...
    /**
     * Bright red outline on the OpenCode conversation tab for [sessionId].
     * A null id removes the outline and its observer. Not a persistent page patch:
     * the JVM installs and removes it with the foreign-session warning.
     */
    fun buildForeignSessionTabOutlineScript(sessionId: String?): String {
        val id = sessionId?.takeIf { OpenCodeServerProtocol.isSessionId(it) }?.let(::escapeJavaScript).orEmpty()
        @Language("JavaScript")
        val script = """
            (() => {
              const STYLE_ID = 'opencode-intellij-foreign-session-outline';
              const ATTR = 'data-opencode-intellij-foreign-session';
              const KEY = '__opencodeIntellijForeignSessionOutline';
              const sessionId = '$id';
              ...
              style.textContent = '[' + ATTR + '="true"]{outline:2px solid #ff1744 !important;outline-offset:1px !important;border-radius:6px;}';
              const parent = document.head || document.documentElement;
              ...
              const apply = () => {
                ...
                const marked = [];
                const slots = document.querySelectorAll('[data-titlebar-tab-slot], [data-titlebar-tab]');
                for (let i = 0; i < slots.length; i++) {
                  const node = slots[i];
                  const slot = node.closest('[data-titlebar-tab-slot]') || node;
                  const key = slot.getAttribute('data-tab-key') || '';
                  const link = slot.querySelector('a[href]');
                  const href = link ? (link.getAttribute('href') || '') : '';
                  if (key.indexOf(sessionId) === -1 && href.indexOf(sessionId) === -1) continue;
                  slot.setAttribute(ATTR, 'true');
                  marked.push(slot);
                }
                const stale = document.querySelectorAll('[' + ATTR + ']');
                for (let i = 0; i < stale.length; i++) {
                  if (marked.indexOf(stale[i]) === -1) stale[i].removeAttribute(ATTR);
                }
              };
              const touchesTabs = (node) => {
                if (!node || node.nodeType !== 1) return false;
                if (node.id === STYLE_ID) return true;
                if (node.matches && node.matches('[data-slot="titlebar-tabs"], [data-titlebar-tab], [data-titlebar-tab-slot]')) return true;
                return !!(node.querySelector && node.querySelector('[data-slot="titlebar-tabs"], [data-titlebar-tab]'));
              };
              let queued = false;
              const observer = new MutationObserver((records) => {
                ...
                for (let i = 0; i < records.length; i++) {
                  ...
                  if (record.target === document.head || touchesTabs(record.target)) {
                    relevant = true;
                    break;
                  }
                  const added = record.addedNodes;
                  for (let j = 0; j < added.length; j++) {
                    if (touchesTabs(added[j])) {
                      relevant = true;
                      break;
                    }
                  }
